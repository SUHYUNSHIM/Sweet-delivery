...
public class Join extends JFrame {
	...
	public Join() {
		...
		Image bgimg = new ImageIcon(Join.class.getResource("/image/joinbg4.png")).getImage();
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(name.length() == 0 || id.length() == 0 || pw.length() == 0 || telNum.length() == 0 ) {
				...
				if(!f.exists()) {
					f.mkdir();
					f1.mkdir();
					System.out.println("MemberJoin폴더가 생성되었습니다.");
				}else {
					System.out.println("폴더가 이미 있습니다.");
				}				
				}
				
			File f = new File(".\\src\\resource\\MemberJoin\\"+id+".txt");
		...
		btnReturn.setIcon(new ImageIcon(Join.class.getResource("/image/return.png")));
